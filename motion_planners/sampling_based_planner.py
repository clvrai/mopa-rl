...
class SamplingBasedPlanner:
    def __init__(self, config, xml_path, num_actions, is_jnt_limited, passive_joint_idx=[], glue_bodies=[], ignored_contacts=[]):
        self.config = config
        ...
        self.non_limited_idx = np.where(np.invert(is_jnt_limited))[0]
