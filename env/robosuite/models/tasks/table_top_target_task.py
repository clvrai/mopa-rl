from env.robosuite.models.tasks import Task, UniformRandomSampler
from env.robosuite.utils.mjcf_utils import new_joint, array_to_string


class TableTopTargetTask(Task):
    """
    ...
    def __init__(self, mujoco_arena, mujoco_robot, mujoco_objects, initializer=None, mujoco_robot_indicator=None, mujoco_target_robot_indicator=None):
        """
        Args:
        ...
        if mujoco_target_robot_indicator is not None:
            self.target_robot_indicator = mujoco_target_robot_indicator
            self.merge(mujoco_target_robot_indicator)
