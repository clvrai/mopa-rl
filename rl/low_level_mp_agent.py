...
class LowLevelMpAgent(SACAgent):
    ...
    def __init__(self, config, ob_space, ac_space, actor, critic, mp_actor):
        super().__init__(config, ob_space, ac_space, actor, critic)

        self._mp_actor = mp_actor
    ...
    def act(self, ob, meta_ac, curr_qpos=None, target_qpos=None, is_train=True, return_stds=False):
        ac = OrderedDict()
        ...
        if self._skills[skill_idx] == 'mp':
            assert curr_qpos is not None and target_qpos is not None, "Current position and target position are None"
            ac = self._mp_actor.plan(curr_qpos, target_qpos)
            activation = None
            stds = None
        else:
