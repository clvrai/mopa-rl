...
simple_planner = PlannerAgent(args, env.action_space, non_limited_idx, passive_joint_idx, ignored_contacts, planner_type=args.simple_planner_type, goal_bias=1.0) # default goal bias is 0.05
...
for episode in range(N):
    ...
    while not done:
        ...
        traj, success, valid, exact = planner.plan(current_qpos, target_qpos, timelimit=args.timelimit)
        ...
        if success:
            for j, next_qpos in enumerate(traj):
                ...
                if np.any(action['default'] < -env._ac_scale) or np.any(action['default'] > env._ac_scale):
                    inner_traj = simple_planner.plan(env.sim.data.qpos.copy(), next_qpos, timelimit=args.simple_planner_timelimit)
